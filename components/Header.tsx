...
const Header = () => {
  ...
  const localDil = localStorage.getItem('secilenDil');


  console.log("localDil: ",localDil);
  console.log("dil: ",dil);
...
<li className='hover:bg-[#352F44] hover:text-[#FAF0E6] px-3 py-2 rounded-xl duration-500 text-[#352F44]'><Link href={`${localDil=="Türkçe"?link.link:link.link2}`} className='flex items-center gap-x-2'> {link.icon}            
{localDil=="Türkçe"?link.isim:link.isim2}</Link></li>
            ))}
...
{localDil=="Türkçe"?"English":"Türkçe"}</button></li>
